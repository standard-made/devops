...
/****** Object:  StoredProcedure [dbo].[sp_STANDARDRegressionCreateContract]]    Script Date: 3/26/2020 11:22:25 AM ******/
IF (OBJECT_ID('sp_STANDARDRegressionCreateContract') IS NOT NULL) DROP PROCEDURE [dbo].[sp_STANDARDRegressionCreateContract]
GO
...
CREATE PROC [dbo].[sp_STANDARDRegressionCreateContract]
AS
BEGIN

	DELETE FROM contract WHERE  CAST(creationdate AS DATE) >= CAST(GETDATE()-7 AS DATE) AND creationname = 'STANDARD Regression Automation'
	DELETE FROM contractacctg WHERE  CAST(creationdate AS DATE) >= CAST(GETDATE()-7 AS DATE) AND creationname = 'STANDARD Regression Automation'
	DELETE FROM contractparty WHERE  CAST(creationdate AS DATE) >= CAST(GETDATE()-7 AS DATE) AND creationname = 'STANDARD Regression Automation'
	...
	SELECT @surrogatemax = MAX(CAST(c.surrogate AS NUMERIC)) + 1 FROM contractparty c; 
	INSERT INTO [dbo].[contractparty] ([contract], [surrogate], [begtime], [endtime], [counterparty], [partytype], [contractpartyclass], [termnotice], [conaddress], [contractprep], [exhibitprep], [invaddress], [notification], [payaddress], [trader], [admin], [operations], [accountant], [other], [creationname], [creationdate], [revisionname], [revisiondate]) VALUES (@contract, @surrogatemax+ 1, @firstdaycm, NULL, 'Metroplex Energy Inc.', 'BUYER/SELLER',NULL, NULL, NULL, 1, 1, NULL, 'EMAIL', NULL, NULL, NULL, NULL, NULL, @count, 'STANDARD Regression Automation', GETDATE(), NULL, NULL)
		,(@contract, @surrogatemax + 2, @firstdaycm, NULL, 'RaceTrac', 'BUYER/SELLER',NULL, NULL, NULL, 1, 1, NULL, 'EMAIL', NULL, NULL, NULL, NULL, NULL, @count, 'STANDARD Regression Automation', GETDATE(), NULL, NULL)
