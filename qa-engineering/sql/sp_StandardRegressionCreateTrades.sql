...
/****** Object:  StoredProcedure [dbo].[sp_STANDARDRegressionCreateTrades]]    Script Date: 3/25/2020 11:22:25 AM ******/
IF (OBJECT_ID('sp_STANDARDRegressionCreateTrades') IS NOT NULL) DROP PROCEDURE [dbo].[sp_STANDARDRegressionCreateTrades]
GO

BEGIN
	EXEC sp_msforeachtable 'ALTER TABLE ? NOCHECK CONSTRAINT all'
		DELETE FROM trade WHERE creationname = 'STANDARD Regression Automation'
		DELETE FROM position WHERE creationname = 'STANDARD Regression Automation'
		DELETE FROM physicalposition WHERE creationname = 'STANDARD Regression Automation'
		DELETE FROM physicalquantity WHERE creationname = 'STANDARD Regression Automation'
		DELETE FROM fee WHERE creationname = 'STANDARD Regression Automation'
...
CREATE PROC [dbo].[sp_STANDARDRegressionCreateTrades] (
	@dealtype VARCHAR(64)
)
AS
BEGIN
	...
	/* INSERTS NEW TPR DEAL TRADE INTO STANDARD THEN ADDS TRADE DETAILS BASED ON LN MAPPING DATA ************/
	/*******************************************************************************************************/
	--START dbo.trade----------------------------------------------------------------------------------------
	---------------------------------------------------------------------------------------------------------
	...
	VALUES (@tradenummax, @dealtype, 0, GETDATE(), 'Metroplex Energy Inc.', CAST(YEAR(GETDATE()) AS VARCHAR(4)) + '-' + FORMAT(GETDATE(),'MM'), @firstdaycm, @lastdaynm, NULL, NULL, NULL, NULL, 'Approved', 0, 'ACTIVE', 'NEW', NULL, NULL, NULL, 'STANDARD Regression Automation', NULL, NULL, 'MONTH', 'USD', 1, 0, NULL, NULL, 0, NULL, @firstdaycm, @lastdaynm, 0, 'US', 0, 1, 0, 0, NULL, NULL, NULL, NULL, NULL, 'STANDARD Regression Automation', 0, 0, 0, 0, NULL, 'STANDARD Regression Automation', GETDATE(), @dealtype, GETDATE(), NULL, 0, 0, 0, NULL, NULL)
	--END dbo.trade------------------------------------------------------------------------------------------
	---------------------------------------------------------------------------------------------------------


	/* start position handling */
	SELECT @trade = MAX(CAST(t.trade AS NUMERIC)) FROM trade t WHERE t.creationname = 'STANDARD Regression Automation' AND t.tradetype = @dealtype
	SELECT @contract = MAX(CAST(c.contract AS NUMERIC)) FROM contract c WHERE c.creationname = 'STANDARD Regression Automation'
	DECLARE @ninjarich AS VARCHAR(64);
	SELECT @ninjarich = COUNT(*) FROM position WHERE creationname = 'STANDARD Regression Automation'
	IF @ninjarich != 0
	  BEGIN
		SET @positionmax = (SELECT MAX(CAST(p.position AS NUMERIC)) + 1 FROM position p WHERE p.creationname = 'STANDARD Regression Automation');
		SET @posdetailmax = (SELECT MAX(CAST(pp.posdetail AS NUMERIC)) + 1 FROM physicalposition pp WHERE pp.creationname = 'STANDARD Regression Automation');
	  END
	ELSE
	  BEGIN
	...
	BEGIN TRY
	  ...
	  VALUES (@positionmax, NULL, @contract, @trade, 'PHYSICAL', 'SELL', NULL /*update later*/, NULL /*update later*/, 'bbl', 'Metroplex Energy Inc.', 'CONV-87-E10', 'Physical Book', NULL, NULL, NULL, NULL, '10 Days After Delivery', 'USD', 0, NULL, NULL, 0.000000, NULL, 'VARIABLE', 'MONTH', NULL, 'gal', 0, 0, 0.000000, 0.0000, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, 'STANDARD Regression Automation', GETDATE(), @dealtype, GETDATE(), 0, NULL, NULL, 0, NULL, NULL, NULL, 0, NULL, 'Open')
	...
	SET @positionmin = (SELECT MIN(CAST(p.position AS NUMERIC)) FROM position p WHERE creationname = 'STANDARD Regression Automation' AND revisionname = @dealtype AND trade = @trade)
	...
	BEGIN TRY
	  ...
	  VALUES (@positionmin, @posdetailmax, 'CONV-87-E10', NULL /*update later*/, NULL, NULL, NULL, NULL, NULL, 100000000, NULL, 'RACK TRUCK', 'FOB', NULL, 0, NULL, NULL, 0, 1.00000000, NULL, NULL, NULL, NULL, NULL, 'USD', NULL, NULL, NULL, 'STANDARD Regression Automation', GETDATE(), @dealtype, GETDATE())	
	...
	SET @posdetailmax = (SELECT MIN(CAST(pp.posdetail AS NUMERIC)) FROM physicalposition pp WHERE pp.creationname = 'STANDARD Regression Automation' AND revisionname = @dealtype AND pp.position IN (SELECT position FROM position WHERE trade = @trade AND revisionname = @dealtype))
	SET @surrogatemax = (SELECT MAX(CAST(pq.surrogate AS NUMERIC) + 1) FROM physicalquantity pq)
	SET @positionmin = (SELECT MIN(CAST(p.position AS NUMERIC)) FROM position p WHERE creationname = 'STANDARD Regression Automation' AND revisionname = @dealtype AND trade = @trade)
	SELECT @positioncountmax = (SELECT COUNT(*) FROM position WHERE creationname = 'STANDARD Regression Automation' AND revisionname = @dealtype AND trade = @trade)
	...
	BEGIN TRY  
	  ...
	  VALUES (@positionmin, @posdetailmax, @surrogatemax, 'DELIVERY', @firstdaycm, @lastdaynm, NULL  , NULL, NULL, NULL, NULL, 'TRADE', 0, 16979.0000, 'mt', 100000.0000, 'bbl', 0.0000, 1.00000000, NULL, 'API', 0.0000, 0, NULL, 'TOTAL', NULL, NULL, NULL, 'STANDARD Regression Automation', GETDATE(), @dealtype, GETDATE(), NULL)
	  , (@positionmin, @posdetailmax, (@surrogatemax + 1), 'DELIVERY', @firstdaycm, @lastdaynm, NULL  , NULL, NULL, NULL, NULL, 'FORECAST', 1, 16979.0000, 'mt', 100000.0000, 'bbl', 0.0000, 1.00000000, NULL, 'API', 0.0000, 0, NULL, 'TOTAL', NULL, NULL, NULL, 'STANDARD Regression Automation', GETDATE(), @dealtype, GETDATE(), NULL)
	...
	BEGIN TRY
	  ...
	  VALUES (@feemax, 'POSITION', @positionmin, 'FIXED', NULL, NULL, 'Metroplex Energy Inc.', NULL /*update later*/, NULL, NULL, NULL, 'gal', NULL, 'AVG', 1.123456, 'USD', NULL, NULL, 'COMMODITY PRICE', NULL, 1.00000000, 1.00000000, 1.00000000, NULL, NULL, @feemax, NULL, NULL, NULL, NULL, NULL, NULL, NULL, 'STANDARD Regression Automation', GETDATE(), @dealtype, GETDATE())
	...
	WHERE pp.creationname = 'STANDARD regression Automation'
