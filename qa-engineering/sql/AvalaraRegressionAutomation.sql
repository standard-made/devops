...
DROP TABLE IF EXISTS dbo.avalaraautomation_findetail_prodtemp; 
DROP TABLE IF EXISTS dbo.avalaraautomation_findetail_uatemp;
...
SELECT * FROM avalaraautomation_findetail_uatemp
--PROD
BEGIN
	SELECT * INTO avalaraautomation_findetail_prodtemp FROM findetail
...
--UA
BEGIN
	SELECT * INTO avalaraautomation_findetail_uatemp FROM findetail
	WHERE posdetail IN (SELECT 
		DISTINCT posdetail
	FROM valuationdetail
...
--COMPARE
SELECT ua.findetail
FROM avalaraautomation_findetail_uatemp ua
WHERE NOT EXISTS (SELECT prod.findetail 
FROM avalaraautomation_findetail_prodtemp prod
...
BEGIN
	...
	UPDATE trade SET begtime = @firstdaycm, begdate = @firstdaycm, endtime = @lastdaynm
		...
		, revisiondate = GETDATE(), revisionname = 'Avalara Regression Automation'
	...
	SELECT @newcontract = MAX(CAST(c.contract AS NUMERIC)) FROM contract c WHERE c.creationname = 'Avalara Regression Automation';

	UPDATE position SET contract = @newcontract, product = 'CONV-87-E10', rt_expecteddeliverymonth = @deliverymonth
	, revisiondate = GETDATE(), revisionname = 'Avalara Regression Automation' 
	WHERE trade IN (SELECT trade FROM trade WHERE revisionname = 'Avalara Regression Automation')
END

--------------------------------------------------------------------------
--------------------------------------------------------------------------
DECLARE @test int
SELECT @test = MAX(CAST(t.trade AS NUMERIC)) FROM trade t WHERE creationdate >= CAST(GETDATE()-1 AS DATE) AND creationname = 'Avalara Regression Automation'
SELECT @test
--------------------------------------------------------------------------
--------------------------------------------------------------------------
...
--PROD
IF (OBJECT_ID('tempdb..#avalaraautomation_findetail_prodtemp') IS NOT NULL) DROP TABLE #avalaraautomation_findetail_prodtemp
...
ORDER BY revisiondate DESC); BEGIN SELECT * INTO #avalaraautomation_findetail_prodtemp FROM findetail
...
--COMPARE
SELECT findetail FROM avalaraautomation_findetail_prodtemp prod WHERE NOT EXISTS
    (SELECT ua.* FROM avalaraautomation_findetail_uatemp ua WHERE prod.findetail = ua.findetail);
SELECT COUNT(findetail) FROM avalaraautomation_findetail_prodtemp prod WHERE NOT EXISTS
    (SELECT ua.* FROM avalaraautomation_findetail_uatemp ua WHERE prod.findetail = ua.findetail);
