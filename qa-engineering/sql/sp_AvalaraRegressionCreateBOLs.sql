...
--/****** Object:  StoredProcedure [dbo].[sp_AvalaraRegressionCreateBOLs]]    Script Date: 3/26/2020 11:22:25 AM ******/
--IF (OBJECT_ID('sp_AvalaraRegressionCreateBOLs') IS NOT NULL) DROP PROCEDURE [dbo].[sp_AvalaraRegressionCreteBOLs]
--GO
...
--CREATE PROC [dbo].[sp_AvalaraRegressionCreateBOLs]
--AS
BEGIN

	DELETE FROM rt_dtnfueladminstaging WHERE creationname = 'Avalara Regression Automation'
	...
	BEGIN
	  ...
	  , [metroplexiscustomer], [processedforallegro], [processed], [pdi_processeddate], [validation], [creationname], [creationdate]
	  , [revisionname], [revisiondate], [mxe_note])
		VALUES (@loadmin, '137137', CAST((ROUND((RAND() * 1000000), 0)) AS varchar(6)) + CONVERT(varchar(8), GETDATE(), 112), @ymd, '', GETDATE(), '', @hms, '', '', '', 600060, 'ENYD', 0000000000, 000137137, 'T', 000137, '', '', '', '', '', '', 137137, '', '', '', '', '', '', '', @ymd, @hms, @ymd, @hms, 'CONV-87-E10', NULL, 'G', 1000, 1000, '', '', '', '', ''/* destination state + location */, '', '', 'D8G', '', '', '', '', '', '', '', '', '', 1, GETDATE(), NULL, NULL, '', 'Avalara Regression Automation', GETDATE(), NULL, NULL, '')
	  SELECT
		@loadmin = @loadmin + 1
	  SELECT
		@loadid = @loadid + 1
